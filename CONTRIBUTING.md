...
To prepare the codebase, install the dependencies with `npm ci`. Then make your changes to the files.

After you have made your changes, run the following commands to verify that the lint, build, and tests pass. `npm run lint` - runs the linter on the codebase. `npm run build` - generates a production build for the application. `npm run test` - runs the unit tests.
